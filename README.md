...
- React 管理台中的实例维护、请求诊断、配置编辑、批准队列和 Snapshot 版本视图。
...
后台产物嵌入 `senixd`，不需要单独部署前端。当前页面支持实例流量/健康状态、手动摘流、按新代次回接、权重调整、二次确认禁用、请求路由诊断、完整配置规划、Owner 批准、受限 Key 管理和审计查看。诊断只读取当前运行快照，不会向业务后端发送模拟请求，原始证据可以直接复制。最后一个可用后端和不健康回接不会被静默绕过；需要操作者显式勾选强制操作。浏览器 Cookie 为 HttpOnly、SameSite=Strict；写操作还要求同源 CSRF 头。退出登录与本机密码重置都会使旧浏览器会话立即失效。
